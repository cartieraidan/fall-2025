...
#define LOOP_INTERVAL_1 100000
#define LOOP_INTERVAL_2 300000
...
static uint16_t loopInterval = (uint16_t) LOOP_INTERVAL_1; //default 4 seconds
	...
	while (1) {
		...
		if (!(P1IN & (uint8_t)(1<<1))) {
			...
			if (RGBstate == 0) { //phase change
				loopInterval = (loopInterval == (uint16_t)LOOP_INTERVAL_1) ? (uint16_t)LOOP_INTERVAL_2 : (uint16_t)LOOP_INTERVAL_1;
			}
